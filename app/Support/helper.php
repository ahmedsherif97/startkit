...
if (! function_exists('get_day_hours')) {
    function get_day_hours(): array
    {
        return [
            '0:00 am', '1:00 am', '2:00 am', '3:00 am', '4:00 am', '5:00 am', '6:00 am', '7:00 am', '8:00 am', '9:00 am', '10:00 am', '11:00 am',
            '0:00 pm', '1:00 pm', '2:00 pm', '3:00 pm', '4:00 pm', '5:00 pm', '6:00 pm', '7:00 pm', '8:00 pm', '9:00 pm', '10:00 pm', '11:00 pm',
        ];
    }
}
if (!function_exists('dates')) {
    function dates($query): string

    {
        ...
        $newTimeStamp = Carbon::parse($oldTimeStamp)->locale(app()->getLocale())->format('g:i a');

        $oldDateStamp = substr($fullCreatedAt, 0, strpos($fullCreatedAt, ' -'));

        $newDateStamp = \Illuminate\Support\Carbon::parse($oldDateStamp)->locale(app()->getLocale())->toDateString();

        $day = Carbon::parse($oldDateStamp)->locale(app()->getLocale())->dayName;
        ...
        $result = $day . ' - ' . preg_replace('/[[:space:]]+-/', ' | ', $fullCreatedAt);
